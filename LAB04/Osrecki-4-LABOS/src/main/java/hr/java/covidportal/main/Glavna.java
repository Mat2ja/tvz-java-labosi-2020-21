...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        // zadatak 1
        System.out.println("\nSortirane bolesti po nazivu pa opisu: ");
        bolesti.stream()
                .sorted(Comparator.comparing(Bolest::getNaziv).thenComparing(Bolest::getOpis))
                .collect(Collectors.toCollection(ArrayList::new))
                .forEach(System.out::println);

        // zadatak 2
        System.out.println("\nPrva bolest po nazivu pa opisu: " + bolesti
                .stream()
                .min(Comparator.comparing(Bolest::getNaziv).thenComparing(Bolest::getOpis))
                .get());

        System.out.println("Zadnja bolesti po opisu pa nazivu: " + bolesti
                .stream()
                .max(Comparator.comparing(Bolest::getOpis).thenComparing(Bolest::getNaziv))
                .get());


        // zadatak 3
        Map<String, List<Bolest>> mapaNazivaBolesti = bolesti
                .stream()
                .collect(Collectors.groupingBy(Bolest::getNaziv));

        System.out.println("\nIspis mape bolesti:");
        for (Map.Entry<String, List<Bolest>> entry : mapaNazivaBolesti.entrySet()) {
    ...
    private static Bolest unosBolesti(Scanner scanner, Set<String> vrsteBolesti, Set<Simptom> simptomi, Set<Bolest> bolesti, int i) {
        ...
        String opis = unosPodatka(scanner, ">> Unesite opis " + (i + 1) + ". bolesti ili virusa: ");
        ...
        Bolest odabranaBolest = switch (vrstaBolesti.toUpperCase()) {
            case "VIRUS" -> new Virus(naziv, odabraniSimptomi, opis);
            default -> new Bolest(naziv, odabraniSimptomi, opis);
        };
