...
import main.java.sample.controllers.PretragaController;
import main.java.sample.controllers.UnosController;
...
public final class BazaPodataka {
    ...
    public static void izbrisiZupaniju(Long id) {
        try (Connection veza = connectToDatabase()) {
            Boolean koristena = BazaPodataka.dohvatiSveOsobe()
                    .stream()
                    .anyMatch(o -> o.getZupanija().getId().equals(id));

            if (koristena) {
                Main.logger.info("Odabrana županija se ne može obrisati.");
                System.out.println("Odabrana županija se ne može obrisati.");
                return;
            }

            String sql = "DELETE FROM ZUPANIJA WHERE ID = ?";
            PreparedStatement upit = veza.prepareStatement(sql);
            upit.setLong(1, id);
            ...
            upit.executeUpdate();
                ...
                SQLException e) {
            Main.logger.error("Greška kod brisanja županije");
            e.printStackTrace();
        }
    }
}