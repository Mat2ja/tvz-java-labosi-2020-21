...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < BROJ_SIMPTOMA; i++) {
            ...
            do {
                try {
                    simptomi[i] = unosSimptoma(scanner, vrijednostiSimptoma, simptomi, i);
                    ponoviPetlju = false;
                } catch (DupliSimptomException ex) {
     ...
     */
    private static Simptom unosSimptoma(Scanner scanner, String[] vrijednostiSimptoma, Simptom[] simptomi, int i) {

        // zadatak 1
        String naziv = "";
        boolean ponoviPetlju;
        do {
            naziv = unosPodatka(scanner, ">> Unesite naziv " + (i + 1) + ". simptoma: ");
            try {
                provjeraNazivaSimptoma(naziv);
                ponoviPetlju = false;
            } catch (NeispravanNazivException ex) {
                logger.error(ex.getMessage(), ex);
                System.out.println("## " + ex.getMessage());
                ponoviPetlju = true;
            }
        ...
        Simptom simptom = new Simptom(naziv, vrijednostSimptoma.toUpperCase());

        provjeraSimptoma(simptom, simptomi);
     ...
     */
    private static Bolest unosBolesti(Scanner scanner, String[] vrsteBolesti, Simptom[] simptomi, Bolest[] bolesti, int i) {
    ...
    /**
     * Provjerava da li je uneseni naziv simptoma validan, ako nije, baca iznimku
     *
     * @param naziv podatak o nazivu simptoma
     * @throws NeispravanNazivException iznimka bačena u slučaju da naziv nije validan
     */
    private static void provjeraNazivaSimptoma(String naziv) throws NeispravanNazivException {
        boolean b = Pattern.matches("^[a-zA-Z](\\s?[a-zA-Z]){2,20}$", naziv);
        if (!b) {
            throw new NeispravanNazivException("Unijeli ste pogrešan naziv simptoma. Molimo pokušajte ponovno!");
        }
    }
    ...
    /**
     * Provjerava da li uneseni simptom već postoji, ako da, baca iznimku
     *
     * @param odabraniSimptom podatak o unesenom simptomu
     * @param simptomi        podatak o svim dostupnim simptomima
     */
    private static void provjeraSimptoma(Simptom odabraniSimptom, Simptom[] simptomi) {
        for (Simptom simptom : simptomi) {
            if (simptom == null) break;
            if (simptom.equals(odabraniSimptom)) {
                throw new DupliSimptomException("Unijeli ste simptom koji već postoji.");
            }
        }
    }
     ...
     */
    private static void provjeraBolesti(Bolest[] bolesti, Bolest odabranaBolest) {
